...
class VWAPBounceStrategy(Strategy):
    ...
    def get_entry_signal(self, data: pd.DataFrame) -> Signal | None:
        ...
        if self._was_above_vwap(data, vwap) and self._is_bullish_candle(data, -1) and rsi.iloc[-1] > 30:
            return Signal(
                timestamp=timestamp,
                symbol=self.symbol,
                side=Side.LONG,
        ...
        if self._was_below_vwap(data, vwap) and self._is_bearish_candle(data, -1) and rsi.iloc[-1] < 70:
            return Signal(
                timestamp=timestamp,
                symbol=self.symbol,
                side=Side.SHORT,
